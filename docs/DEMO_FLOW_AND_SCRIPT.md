...
The Large Language Model (LLM) is an intelligent reasoning agent, but **it never has unrestricted authority over financial decisions**. Every action must pass strict deterministic financial policies before execution.
...
flowchart TD
    ...
    I -->|If Failed and Retries Left| E
...
| **3. Economic Decision Engine** | `src/decision/` | Calculates **Expected Net Value (ENV)**:<br>`ENV = (Amount × P(recovery)) − Intervention Cost`<br>Filters actions by eligibility and ensures the business never loses money attempting a recovery. |
...
* **What to say:**
  ...
  > 
  > *RecoveryOS brings together failure intelligence, calibrated machine learning, economic evaluation, and an AI agent governed by strict, deterministic financial guardrails. Let's walk through how it works."*
...
* **What to point out on the Decision Trace screen:**
  ...
  2. **Step 2: AI Decision (Agent Proposal)**
     ...
     - Highlight the **Rationale** generated by the AI agent explaining why this action was chosen.
  3. **Step 3: Deterministic Policy Guardrail**
     - Show the **Guardrail Check: ALLOWED** banner (or DENIED if limits reached).
     - **Emphasize the core architectural message:** The AI is only an advisor; the policy engine has veto authority.
  4. **Step 4: Execution & Final Outcome**
     - Show the Execution Status (`SUCCESS`) and Final Status (`RECOVERED`).
* **What to say:**
  > *"This is the **Decision Trace**, the core showcase of RecoveryOS's architecture. It makes the AI's reasoning completely transparent and auditable.
  > 
  ...
  > 
  > Third, our LangGraph AI agent reasons over this context and proposes the optimal recovery action with an operational rationale.
  > 
  > Fourth—and most importantly—notice this section: **Deterministic Policy Guardrails**. In a regulated financial environment, you cannot let an LLM make unconstrained financial choices. Our deterministic policy engine verifies that the action respects hard limits: cooldown periods, max retries, and fraud checks.
  > 
